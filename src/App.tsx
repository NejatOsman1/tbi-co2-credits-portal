...
/** Full model covers all substeps (dummy fields included) */
export const FullSchema = z.object({
  /** Prescan */
  prescanProjectNaam: z.string().optional(),
  prescanContact: z.string().optional(),

  /** Quick scan -> Product en Materialen (your existing list) */
  quickScan: z
    .array(
      z.object({
        typeProduct: z.enum(typeProductOptions),
        fabrikant: z.enum(typeProductOptions),
  ...
  /** Project validatie -> Projectplan template invullen (dummy) */
  projectplanTitel: z.string().optional(),
  projectplanBeschrijving: z.string().optional(),

  /** Project validatie -> Upload bewijsstukken (dummy as URLs) */
  bewijsLinks: z.array(z.string().url("Voer een geldige URL in")).optional(),

  /** Project validatie -> Resultaat validatie Oncra (dummy) */
  validatieGoedgekeurd: z.boolean().optional(),
  validatieToelichting: z.string().optional(),

  /** Publiceer -> Upload projectfoto's (dummy as URLs) */
  fotoUrls: z.array(z.string().url("Voer een geldige URL in")).optional(),

  /** Publiceer -> Selecteer verkoopkanalen (dummy) */
  verkoopKanaal: z
    .enum(["Marketplace", "Direct", "Veiling"] as const)
    .optional(),

  /** Publiceer -> Onderhands verkoop registreren (dummy) */
  koperNaam: z.string().optional(),
  verkoopBedrag: z.number().optional(),

  /** Publiceer -> Overzicht (no fields; just for review) */
});
  ...
  quickScan: z
    .array(
      z.object({
        ...
        fabrikant: z.enum(typeProductOptions, { required_error: "Kies een fabrikant" }),
        aantal: z.number().positive({ message: "Voer een positief getal in" }),
        eenheid: z.string().min(1, "Verplicht veld"),
      })
    )
...
const prescanSchema = z.object({
  prescanProjectNaam: z.string().min(1, "Projectnaam is verplicht"),
  prescanContact: z.string().min(1, "Contact is verplicht"),
});
  ...
  return (
    <Box>
      <Typography variant="subtitle2" sx={{ mb: 1 }}>
        Product en Materialen — voeg items toe
      </Typography>
      ...
      <ListField name="quickScan">
        <ListItemField name="$">
          ...
          >
            <SelectField name="typeProduct" label="Type product" allowedvalues={typeProductOptions as unknown as string[]} fullWidth/>
            <SelectField name="fabrikant" label="Fabrikant" allowedvalues={typeProductOptions as unknown as string[]} fullWidth/>
...
function PendingCircle({ active }: { active?: boolean }) {
  return (
    <RadioButtonUncheckedIcon
      fontSize="small"
      sx={{ color: active ? textColour : undefined }}
    />
  );
}
...
type SubstepDef = {
  ...
  /** render custom body instead of AutoFields */
    ...
    substeps: [
      {
        key: "prescan-main",
        label: "Prescan",
        fields: ["prescanProjectNaam", "prescanContact"],
        zod: prescanSchema,
      },
    ],
  },
  ...
  },
  {
    ...
    substeps: [
      ...
      },
      {
        key: "overzicht",
        label: "Overzicht",
        fields: [], // no inputs; we'll show Review at the end of content pane
        zod: null,
      },
    ],
  },
...
function Sidebar({
  ...
  const currentFlatIndex =
    steps.slice(0, activeStep).reduce((sum, s) => sum + s.substeps.length, 0) + activeSub;
    ...
    <Box sx={{ p: 3, borderRight: { md: "1px solid" }, borderColor: { md: "divider" } }}>
      ...
      <List dense>
        {steps.map((step, si) => {
          ...
          const stepActive =
            currentFlatIndex >= stepStartIndex && currentFlatIndex <= stepEndIndex;
                  ...
                  >
                    <ListItemIcon sx={{ minWidth: 28 }}>
                      {isDone(flatIdx) ? (
                        ...
                        <PendingCircle active={isActive(flatIdx)} />
                      )}
                    </ListItemIcon>
                    <ListItemText
                      primary={sub.label}
...
export default function App(): JSX.Element {
  ...
  const [model, setModel] = useState<FormModel>({
    quickScan: [{ typeProduct: "Zonnepanelen", fabrikant: "Zonnepanelen", aantal: 1, eenheid: "st." }],
  });
  ...
  const goNext = () => {
    // advance sub within step; if at end, advance step
  ...
  /** Title + description per substep (just simple helper text) */
    ...
    <ThemeProvider theme={theme}>
      <Container maxWidth="lg" sx={{ py: 6 }}>
        <Paper elevation={2} sx={{ p: 0, overflow: "hidden", borderRadius: 3 }}>
          <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "300px 1fr" } }}>
            {/* Sidebar */}
